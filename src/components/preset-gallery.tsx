      ...
      {presets.map((preset, i) => {
        ...
        return (
          <article
            key={preset.id}
            className={cn(
              "glass group relative z-0 flex min-h-16 items-center gap-3 overflow-hidden rounded-2xl px-3 py-2.5 transition-all duration-300",
          ...
          >
            ...
            />
            <span className="relative z-10 flex size-9 shrink-0 items-center justify-center rounded-xl bg-white/4 text-cyan">
            ...
            <Button
              size="icon"
              className="relative z-10 size-8 shrink-0 rounded-full"
              onClick={(e) => {
                e.stopPropagation();
                onRun(preset.id);
              }}
