...
export function DemoApp({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      ...
      <main className="mx-auto flex min-h-0 w-full max-w-6xl flex-1 flex-col gap-1.5 overflow-y-auto overflow-x-hidden px-3 pt-1.5 pb-2 safe-pad-b sm:gap-3 sm:px-6 sm:pt-2.5 sm:pb-3 md:overflow-hidden">
        <FlowStepper
          current={step}
          view={view}
        ...
        <div key={view} className="stage-in flex min-h-0 flex-1 flex-col md:overflow-hidden">
          ...
          {view === 2 ? (
            <div className="flex min-h-0 flex-1 flex-col gap-3 md:overflow-hidden">
              ...
              <div className="relative z-0 grid min-h-0 flex-1 gap-3 md:grid-cols-2 md:overflow-hidden">
                ...
                />
                <div className="min-h-0 flex flex-col md:h-full">
                <MarketplacePanel
                  query={query}
                  onQuery={setQuery}
                  category={category}
          ...
          {view === 3 ? (
            plan ? (
              <section className="flex min-h-0 flex-1 flex-col gap-2 md:grid md:grid-cols-[minmax(0,1.5fr)_minmax(16rem,0.85fr)] md:gap-3 md:overflow-hidden">
                <FlowTimeline
                  plan={plan}
                  executing={executing}
                  network={network}
